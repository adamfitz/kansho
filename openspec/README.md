...
| [Site Plugin System](specs/site-plugin-system/spec.md) | `specs/site-plugin-system/spec.md` | Plugin interface, extraction methods (JS/selector/custom/api), Go-driven pagination (NextPageJS), scroll-to-load images, custom extraction timeouts, site registry, config, filename normalization, CF bypass support, API client User-Agent, filtering of non-chapter assets from image lists, full server-rendered chapter list on manga pages (arenascan), chapter images inside a noscript fallback (arenascan) |
| [Download Manager](specs/download-manager/spec.md) | `specs/download-manager/spec.md` | Download lifecycle, chapter/image fetch orchestration, CBZ creation, per-site retry policy (RetryPolicySite / SiteRetryPolicy) covering chapter, image, and fetch-level retries, decaying backoff that steps down on success instead of resetting (skip CF challenges), Referer-protected image downloads (comix), context cancellation including in-flight image requests |
| [Download Queue](specs/download-queue/spec.md) | `specs/download-queue/spec.md` | FIFO queue singleton, single-chapter tasks, task states (incl. waiting_cf / skipped_cf), cancellation (single + all + per-manga queued-only), CF challenge handling (queue pause, single browser window, wait-for-data timeout, skip CF tasks, auto-resume via ResumeCfTasks), retry (failed/cancelled/CF-blocked/skipped), UI callbacks |
...
| [Browser Automation](specs/browser-automation/spec.md) | `specs/browser-automation/spec.md` | Chromedp session management, JS evaluation, Go-driven pagination and scroll-to-load, extraction timeouts, CF cookie injection, CF challenge detection, batched HTML fetching |
| [User Interface](specs/user-interface/spec.md) | `specs/user-interface/spec.md` | Fyne layout (2-column, gradient), centralized state with observer callbacks, 3-pane chapter list with per-chapter progress and green-tick/download-arrow/red-X controls, local-only chapter loading with manual remote refresh, SQLite-backed total/downloaded/not-downloaded chapter counts plus saved `last_refresh` dates shown in the main status bar, download queue summary pop-up with "Currently Downloading" split view (chapter + live progress + Stop) and per-task Start/Retry + per-manga Cancel Queue, CF dialog + auto-resume of blocked downloads after data import, menus, keyboard shortcuts, views |
...
| [MangaDex Title Info](specs/mangadex-title-info/spec.md) | `specs/mangadex-title-info/spec.md` | Per-manga info (i) button, MangaDex title lookup dialog (50-result pages + infinite scroll + manual URL/ID), info pane in the chapter list card with copyable fields and clickable link, language dropdown with English default, local SQLite database shared with per-manga chapter-count snapshots, Database menu (Backup/Restore/Compact) |
