...
func Test_FlameComics_Series2_Chapters_And_Images(t *testing.T) {
    ctx, cancel := context.WithTimeout(context.Background(), 90*time.Second)
    defer cancel()

    site := &sites.FlameComicsSite{}

    // Stable, long-running FlameComics series
    const mangaURL = "https://flamecomics.xyz/series/2" // Omniscient Reader's Viewpoint
    ...
    for _, img := range images {
        if !strings.HasPrefix(img, "https://cdn.flamecomics.xyz/") &&
