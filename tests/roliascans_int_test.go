...
func Test_Roliascans_Chapters_And_Images(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
	defer cancel()

	site := &sites.RoliascansSite{}

	// Stable roliascan.com series
	const mangaURL = "https://roliascan.com/manga/my-bias-gets-on-the-last-train/"
	...
	log.Printf("[TEST] SUCCESS — roliascans scraper is working")
}