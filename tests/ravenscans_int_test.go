...
func Test_RavenScans_NanoMachine_Chapters_And_Images(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), 90*time.Second)
	defer cancel()

	site := &sites.RavenscansSite{}

	// Stable, long-running RavenScans series
	const mangaURL = "https://ravenscans.org/manga/dead-end-demonics-the-first-returner/"
	...
	for _, img := range images {
		if !strings.HasPrefix(img, "https://cdn") ||
			!strings.Contains(img, ".ravenscans.org/") {
			t.Fatalf("Invalid image URL: %s", img)
		}
	}

	log.Printf("[TEST] SUCCESS — RavenScans scraper is working")
}
